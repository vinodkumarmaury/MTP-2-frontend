...
const PARAM_DOCS: Record<string, ParamDoc> = {
  // ── Economic ──────────────────────────────────────────────────────────────
  npv_cr: {
    desc: 'Sum of all discounted future mine cash flows minus initial CAPEX. Primary investment decision metric — higher NPV = more value created.',
    formula: 'NPV = Σ[CF_t/(1+r)^t] − CAPEX₀. CMPDI hurdle: NPV>0 at 10–12% discount rate.',
    scoring: '0–100: NPV₁₀₀₀Cr=100. Weight: 28% of Economic score.',
    benchmark: 'Kuju OCP: ₹280 Cr | Gevra OCP: ₹48,000 Cr | Rajmahal: ₹6,800 Cr',
  },
  irr_pct: {
    desc: 'Discount rate at which NPV=0. Compared against WACC; positive spread means the project creates shareholder value.',
    formula: 'Solve NPV(r)=0 via Newton–Raphson. Score = 50 + (IRR−WACC)×5.',
    scoring: 'Weight: 24% of Economic score. IRR=WACC scores 50.',
  ...
  payback_period_yr: {
    ...
    formula: 'PBP = CAPEX₀ / Annual Operating Cash Flow. Score: (10−PBP)/(10−2)×100.',
    scoring: '2 yr=100, 10 yr=0. Weight: 16% of Economic score.',
    benchmark: 'Kuju OCP: 2.8 yr | Gevra: 1.8 yr | India OC avg: 4–7 yr',
  },
  capex_cr: {
    desc: 'Total pre-production capital: civil works, HEMM, electrical, EPCM fee, contingency, owner\'s cost.',
    formula: 'CAPEX = Civil + Mechanical + Electrical + EPCM% + Contingency% + Owner\'s cost.',
  ...
  sustaining_capex_cr: {
    desc: 'Annual capital for HEMM replacement, OB dump raising, exploration, infrastructure maintenance.',
    formula: 'Benchmark: 5–10% of initial CAPEX/yr. Reduces free cash flow in later mine years.',
    scoring: 'Indirect — reduces margin score via cash flow model.',
    benchmark: 'Kuju: ₹18 Cr/yr | Industry: 5–10% of initial CAPEX',
  },
  opex_per_tonne: {
    desc: 'All-in cost to mine, process, and despatch one tonne of coal including mining, washing, G&A.',
    formula: 'OPEX/t = Drill+Blast + Load+Haul + Processing + G&A + Royalty + Cess.',
  ...
  ob_mining_cost: {
    desc: 'Cost per BCM to drill, blast, load, haul, and dump overburden — the key driver of total mining cost.',
  ...
  coal_price_t: {
    desc: 'CIL-notified sale price for the declared coal grade. Grade E (4201–4940 kcal/kg) ≈ ₹2,200/t.',
  ...
  wacc_pct: {
    desc: 'Weighted Average Cost of Capital: blended cost of equity and after-tax debt. Sets the IRR hurdle rate.',
    formula: 'WACC = (E/V)×Re + (D/V)×Rd×(1−T). Build-up method for unlisted CIL subsidiaries.',
    scoring: 'Used to compute IRR spread. Higher WACC = lower IRR score.',
    benchmark: 'CMPDI: 10–12% | Kuju: 14% | Gevra: 12%',
  },
  discount_rate_pct: {
    desc: 'Rate used to discount future cash flows in NPV calculation. Usually equals WACC.',
    formula: 'Same as WACC for most analyses. Used in all NPV/DCF calculations.',
    scoring: 'Linked to WACC; affects NPV score.',
    benchmark: 'CMPDI minimum FIRR: 10–12%',
  },
  besr: {
    desc: 'Stripping ratio at which OC mining cost equals underground mining cost. If SR > BESR, UG becomes cheaper.',
    formula: 'BESR = (UG OPEX − OC OPEX) / OB Mining Cost per BCM.',
  ...
  closure_bond_cr: {
    desc: 'Financial provision for post-closure rehabilitation, monitoring, and environmental remediation. NPV of closure liabilities.',
  ...
  coal_grade: {
    desc: 'CIL notified coal grade based on Gross Calorific Value. Grade A (>6700) to Grade G (≤3360 kcal/kg).',
  ...
  debt_equity_ratio: {
    desc: 'Ratio of total debt to equity in mine financing. Lower D/E = less financial leverage risk.',
    formula: 'D/E = Total Debt / Shareholders\' Equity. 0 = fully equity financed.',
  ...
  stripping_ratio_overall: {
    desc: 'Total overburden volume divided by total recoverable coal for full mine life. Lower SR = more economic OC operation.',
    formula: 'OSR = Total OB (BCM) / Total Coal (t). OB Vol = Area × OB Thickness.',
    scoring: '(BESR−SR)/BESR × 100. Weight: 15% of Technical.',
    benchmark: 'Kuju: 4.2 | Gevra: 1.08 (exceptional) | Rajmahal: 5.5 | Haile: 7.9',
  },
  hemm_availability: {
    desc: 'Percentage of scheduled hours when HEMM is ready for operation (not under breakdown or scheduled maintenance).',
  ...
  hemm_utilisation: {
    desc: 'Percentage of available hours when HEMM is actively operating, excluding idle and standby time.',
  ...
  recovery_pct: {
    desc: 'Percentage of geological coal extracted and entering ROM stockpile. Lost to dilution, floor/roof losses.',
  ...
  dilution_oc_pct: {
    desc: 'Percentage of non-coal material mixed into ROM due to mining methods — reduces effective GCV.',
    formula: 'Dilution% = (ROM − In-situ coal tonnes) / ROM × 100.',
    scoring: 'Higher dilution degrades effective GCV. Affects coal quality.',
    benchmark: 'OC coal India: 2–5% | Rajmahal: 2.7%',
  },
  highwall_fos: {
    desc: 'Ratio of resisting to driving forces for the OC highwall slope. Bishop\'s Simplified Method for circular failure.',
    formula: 'FoS = ΣResisting Moments / ΣOverturning Moments (Bishop\'s). Must be ≥1.3.',
    scoring: '(FoS−1.0)/(2.5−1.0)×100. Weight: 9% of Technical; also in Risk.',
    benchmark: 'DGMS minimum: 1.3 | Rajmahal: 1.33 | Gevra: 1.42',
  },
  annual_prod_mty: {
    desc: 'Tonnes of coal produced and despatched per year. Determines revenue, mine life, and HEMM fleet size.',
    formula: 'Production = Shovel Output × Shovels × Hours × Availability × Util / (OSR+1).',
    scoring: 'Log-normalised: 0.5 MTY=0, 50 MTY=100. Weight: 7% of Technical.',
    benchmark: 'Kuju: 1.3 MTY | Gevra: 49 MTY | Rajmahal: 9 MTY',
  },
  mine_life_yr: {
    desc: 'Years until proved reserves are exhausted at planned production. Affects NPV horizon and investment timeline.',
    formula: 'Mine Life = Reserve (Mt) / Annual Production (MTY). Cross-check with LoM schedule.',
    scoring: '(Life−5)/(50−5)×100. Weight: 11% of Technical.',
    benchmark: 'Kuju: 20 yr | Gevra: 25 yr | Rajmahal: 19 yr',
  },
  fuel_consumption_ltr_t: {
    desc: 'Diesel consumed per tonne of total material moved (coal + OB). Key operational efficiency and GHG driver.',
    formula: 'Fuel L/t = Total diesel / (Production × (1+SR)) in tonnes moved.',
  ...
  // ── Geological ────────────────────────────────────────────────────────────
  reserve_mt: {
    desc: 'Proved + Probable Mineral Reserves per UNFC (India) or JORC/NI 43-101. Foundation of all economic modelling.',
  ...
  gcv_blended: {
    desc: 'Heat energy per kg of coal when burned completely. Determines CIL grade and notified price.',
    formula: 'Blended GCV = Σ(Seam GCV_i × Production weight_i). Measured by bomb calorimeter (IS 1350).',
    scoring: '(GCV−2200)/(6700−2200)×100. Weight: 19% of Technical.',
    benchmark: 'Kuju: 3850 kcal/kg (Grade F) | BCCL N.Karanpura: 6800 (Grade B) | Rajmahal: 5200 (Grade E)',
  },
  ash_pct: {
    desc: 'Weight% of non-combustible mineral matter. High ash lowers GCV, grade, and price. >34% requires washing.',
  ...
  seam_thickness_avg_m: {
    desc: 'Average coal seam thickness weighted by areal extent. Thicker seam = lower SR = better economics.',
  ...
  // ── Environmental ─────────────────────────────────────────────────────────
  ec_status: {
    desc: 'Environmental Clearance under Environment Protection Act 1986 / EIA Notification 2006. Without EC, mine cannot operate.',
  ...
  fc_status: {
    desc: 'Forest Clearance under Forest Conservation Act 1980 for diversion of forest land within mine lease.',
  ...
  ob_dump_fos: {
    desc: 'Factor of safety for overburden dump slope stability. Low FoS = dump failure risk, environmental damage.',
    formula: '(FoS−1.0)/(2.0−1.0)×100. DGMS minimum: 1.3. OB dump failure = environmental disaster.',
  ...
  ghg_intensity: {
    desc: 'Scope 1+2 greenhouse gas emissions per tonne of coal produced (tCO₂e/t). Coal mining contributes ~0.04–0.10 tCO₂e/t.',
  ...
  pm10_ambient_ugm3: {
    desc: 'Ambient particulate matter <10μm at mine boundary. CPCB standard ≤100 μg/m³ (annual mean).',
    formula: 'Score = 100 − (PM10−60)/(200−60)×100. <60=excellent, >200=0.',
  ...
  // ── Social ────────────────────────────────────────────────────────────────
  ltifr: {
    desc: 'Lost Time Injury Frequency Rate = number of LTIs per million man-hours worked. Key DGMS safety metric.',
    formula: 'LTIFR = (No. LTIs × 10⁶) / Total man-hours. OC coal India benchmark: 6–12.',
    scoring: '100 − (LTIFR−4)/(20−4)×100. Weight: 24% of Social.',
    benchmark: 'Kuju OCP: ~8 | India OC average: ~9 | World benchmark: <4',
  },
  far: {
    desc: 'Fatal Accident Rate = fatalities per 100 million man-shifts. India coal mining FAR has declined from ~200 to ~15 over 40 years.',
    formula: 'FAR = (Fatalities × 10⁸) / Total man-shifts worked.',
  ...
  csr_spend_cr: {
    desc: 'Annual CSR expenditure under Companies Act 2013 (Section 135). 2% of net profit mandatory for large companies.',
  ...
  contractor_ltifr: {
    desc: 'LTIFR specifically for contract workers. Typically 1.5–2× higher than direct employees due to lower training.',
    formula: 'Contractor LTIFR = (Contractor LTIs × 10⁶) / Contractor man-hours.',
