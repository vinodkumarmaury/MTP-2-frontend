...
const FEATURES = [
  ...
  { href:'/compare',     icon:'📊', label:'Compare',     desc:'Scatter plot + residual table: Mine Composite Index (MCI) predicted vs actual expert scores for 4 validation mines. Mean Absolute Error (MAE) & R² shown.' },
  ...
  { href:'/parameters',  icon:'📖', label:'Parameters',  desc:'All 150+ parameters documented: definition, relevance to the Composite Mine Evaluation Model (CMEM), formula, and source reference mine.' },
    ...
    <div className="space-y-8">
      ...
      <div className="bg-[#1F3864] rounded-2xl p-8 md:p-10 text-white">
        ...
        <p className="text-blue-100 text-base leading-relaxed max-w-3xl mb-2">
          A data-driven system that evaluates Open-Cast (OC) coal mines across <strong>7 dimensions</strong> using over{' '}
          <strong>150 parameters</strong>. The application uses an ensemble weighting approach to predict a{' '}
          Mine Composite Index (MCI), providing objective, reproducible scores validated on real-world mines
          with a Mean Absolute Error of 2.53 points and R² of 0.91.
        </p>
        <p className="text-blue-300 text-sm mb-6">
          <strong>v3.0</strong> introduces <span className="text-white font-semibold">Governance</span> as the
          7th dimension, covering ISO 14001 (Environmental Management) / ISO 45001 (Occupational Health &amp; Safety)
          status, Directorate General of Mines Safety (DGMS) compliance, Environmental, Social, and Governance
      ...
      {/* API Health + Stats strip — client component handles its own state */}
      <HomeStatsStrip />
